...
class ReposeRecordTest(unittest.TestCase):
    def test_find_guard_with_most_sleep(self):
        self.assertEqual(find_guard_with_most_sleep(), '#10')

    def test_find_minute_with_most_sleep(self):
        self.assertEqual(find_minute_with_most_sleep('#10'), 24)

    def test_make_repose_record(self):
        record = add_up_repose_records(TEST_INPUT)
        self.assertEqual(record['#10'][4], 0)
        self.assertEqual(record['#10'][5], 1)
        self.assertEqual(record['#10'][24], 2)
